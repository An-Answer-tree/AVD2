...
class VisionLanguageTSVDataset(object):
    ...
    def get_row_from_tsv(self, tsv, img_idx):
        row = tsv[img_idx]
        print(f"row[0]: {row[0]}")
        print(f"self.image_keys[img_idx]: {self.image_keys[img_idx]}")
        
        row_key = row[0].split('/')[0] if '/' in row[0] else row[0]
        img_key = self.image_keys[img_idx]
        
        # 去掉 img_key 和 row_key 中可能的前缀
        row_key_suffix = row_key.split('_')[-1].lstrip('0')
        img_key_suffix = img_key.split('_')[-1].lstrip('0')
        
        print(f"Processed row_key_suffix: {row_key_suffix}")
        print(f"Processed img_key_suffix: {img_key_suffix}")
        
        assert row_key_suffix == img_key_suffix, f"row_key_suffix: {row_key_suffix}, img_key_suffix: {img_key_suffix}"
    ...
    def get_car_info(self, img_key):
        ...
        all_infos = h5py.File(info_path, 'r')
        print(f"Loading {info_path}, keys: {list(all_infos.keys())}")
        
        info_meta_data = torch.zeros((sensor_type_num, self.decoder_num_frames))
        for key in all_infos.keys():
            try:
                shape = all_infos[key].shape
            except AttributeError:
                print(f"Key: {key} is not a dataset but a group. Skipping...")
                continue
    
            print(f"Key: {key}, shape: {shape}")
            if key in all_choices:
                data_len = shape[0]
                if data_len < self.decoder_num_frames:
                    print(f"Warning: {key} has less data points ({data_len}) than required frames ({self.decoder_num_frames}). Filling with default values.")
                    infos.append(torch.zeros(self.decoder_num_frames))  # 填充默认值
                    continue
                
                samp_id = sampling(0, data_len - 1, self.decoder_num_frames)
                if key == 'speed':
        ...
        if infos:
            ...
            info_meta_data = -1 * torch.ones((sensor_type_num, self.decoder_num_frames))
        
        assert info_meta_data.shape == (sensor_type_num, self.decoder_num_frames), \
            f"Shape mismatch: expected {(sensor_type_num, self.decoder_num_frames)}, got {info_meta_data.shape}"
